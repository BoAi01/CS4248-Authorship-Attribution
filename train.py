...
#     # training setup
#     num_epochs, base_lr, base_bs, ngpus = 2, 2e-5, 64, 4
...
# from models import LogisticRegression
from tqdm import tqdm
from bert_embedding import BertEmbeddings
...
def train_bert(nlp_train, nlp_test):
    ...
    # training setup
    num_epochs, base_lr, base_bs, ngpus = 3, 1e-4, 64, 1
    out_dim = max(test_y) + 1
    ...
    train_loader = DataLoader(train_set, batch_size=base_bs * ngpus, shuffle=True,
                              pin_memory=False)
    test_loader = DataLoader(test_set, batch_size=base_bs * ngpus, shuffle=False,
                             pin_memory=False)
    ...
    with torch.no_grad():
        for x1, x2, y in train_loader:
            bert_train_feat.append(model((x1, x2), return_feat=True)[-1])
        for x1, x2, y in test_loader:
            bert_test_feat.append(model((x1, x2), return_feat=True)[-1])
    return torch.cat(bert_train_feat, dim=0), torch.cat(bert_test_feat, dim=0)
...
def train_style_based(nlp_train, nlp_test):
    ...
    columns = ["avg_len", "num_short_w", "per_digit", "per_cap", "f_a", "f_b", "f_c", "f_d", "f_e", "f_f", "f_g", "f_h",
               "f_i", "f_j", "f_k", "f_l", "f_m", "f_n", "f_o", "f_p", "f_q", "f_r", "f_s", "f_t", "f_u", "f_v", "f_w",
               "f_x", "f_y", "f_z", "f_0", "f_1", "f_2", "f_3", "f_4", "f_5", "f_6", "f_7", "f_8", "f_9", "f_e_0",
               "f_e_1", "f_e_2", "f_e_3", "f_e_4", "f_e_5", "f_e_6", "f_e_7", "f_e_8", "f_e_9", "f_e_10", "f_e_11",
               "richness"]
    ...
    X_style_trains = []
    X_style_tests = []
    for idx in X_style_train.itertuples():
        x = ' '.join([str(getattr(idx, item)) for item in columns])
        encoding = tokenizer(x, return_tensors='pt', padding=True, truncation=True)
        embds = embedding(encoding['input_ids'])
        X_style_trains.append(torch.mean(embds, dim=1).squeeze(1))
    for idx in X_style_test.itertuples():
        x = ' '.join([str(getattr(idx, item)) for item in columns])
        encoding = tokenizer(x, return_tensors='pt', padding=True, truncation=True)
        embds = embedding(encoding['input_ids'])
        X_style_tests.append(torch.mean(embds, dim=1).squeeze(1))
    ...
    # score_style = accuracy_score(nlp_test['Target'], y_pred)
    # f1_style = f1_score(nlp_test['Target'], y_pred, average="macro")
    return torch.cat(X_style_trains, dim=0), torch.cat(X_style_tests, dim=0)
...
def train_char_ngram(nlp_train, nlp_test, list_bigram, list_trigram):
    ...
    feats_trains = []
    feats_tests = []
    for idx in feats_train.itertuples():
        x = []
        for j in range(1, len(feats_train.loc[0]) + 1):
            x.append(str(getattr(idx, f'_{j}')))
        encoding = tokenizer(' '.join(x), return_tensors='pt', padding=True, truncation=True)
        embds = embedding(encoding['input_ids'])
        feats_trains.append(torch.mean(embds, dim=1).squeeze(1))
    for idx in feats_test.itertuples():
        x = []
        for j in range(1, len(feats_test.loc[0]) + 1):
            x.append(str(getattr(idx, f'_{j}')))
        encoding = tokenizer(' '.join(x), return_tensors='pt', padding=True, truncation=True)
        embds = embedding(encoding['input_ids'])
        feats_tests.append(torch.mean(embds, dim=1).squeeze(1))
    # clf_char = xgb.XGBClassifier().fit(feats_train, nlp_train['Target'])
    # clf_char = LogisticRegression(random_state=0).fit(feats_train, nlp_train['Target'])
    # y_pred = clf_char.predict(feats_test)
    # char_prob_test = clf_char.predict_proba(feats_test)
    ...
    return torch.cat(feats_trains, dim=0), torch.cat(feats_tests, dim=0)
...
def run_iterations(source):
    ...
    for limit in list_senders:
        ...
        # Bert + Classification Layer
        bert_train_feat, bert_test_feat = train_bert(nlp_train, nlp_test)
        bert_train_feat = bert_train_feat.cuda()
        bert_test_feat = bert_test_feat.cuda()

        # Style-based classifier
        style_train_feat, style_test_feat = train_style_based(nlp_train, nlp_test)
        style_train_feat = style_train_feat.cuda()
        style_test_feat = style_test_feat.cuda()

        # Character N-gram only
        ngram_train_feat, ngram_test_feat = train_char_ngram(nlp_train, nlp_test, list_bigram, list_trigram)
        ngram_train_feat = ngram_train_feat.cuda()
        ngram_test_feat = ngram_test_feat.cuda()

        # BERT + Style + Char N-gram
        print("#####")
        print("BERT + Style + Char N-gram")
        ensemble_train_feats = torch.cat([bert_train_feat, style_train_feat, ngram_train_feat], dim=-1)
        ensemble_train_feats = ensemble_train_feats.cuda()

        ensemble_test_feats = torch.cat([bert_test_feat, style_test_feat, ngram_test_feat], dim=-1)
        ensemble_test_feats = ensemble_test_feats.cuda()

        multihead_attention = MultiHeadAttention(768 * 3)
        multihead_attention = multihead_attention.cuda()
        ensemble_train_feats = multihead_attention(ensemble_train_feats.unsqueeze(1)).cpu().detach().squeeze(1).detach().numpy()
        ensemble_test_feats = multihead_attention(ensemble_test_feats.unsqueeze(1)).cpu().detach().squeeze(1).detach().numpy()
        clf = LogisticRegression().fit(ensemble_train_feats, nlp_train['Target'])
        y_pred = clf.predict(ensemble_test_feats)
        score_comb_fin = accuracy_score(nlp_test['Target'], y_pred)
        ...
        # Store scores
        list_scores.append([limit, score_comb_fin])
